...
pub fn options_manifest(onename: String, reference: String) -> Json<JsonValue> {
    let _ = onename;
    let _ = reference;
    Json(json!({}))
}
...
pub fn options_manifest_2level(user: String, repo: String, reference: String) -> Json<JsonValue> {
    let _ = repo;
    let _ = user;
    let _ = reference;
    Json(json!({}))
